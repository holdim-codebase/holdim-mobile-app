...
import { Text, View, Button, Image, ScrollView, TouchableWithoutFeedback, Alert, ActivityIndicator } from 'react-native'
          ...
          <TouchableWithoutFeedback key={i} onPress={() => openProposal(item)}>
            <View style={styles.proposalWrapper}>
              ...
              <View style={styles.proposalContentWrapper}>
                ...
                <Text style={styles.proposalDescription}>{item.description.short}</Text>
                <Text style={styles.proposalEndtime}>{dateNow < item.end ? 'Ends:' : 'Voting ended on'} {moment.unix(item.end).format('MMM DD, YYYY, hh:mm')}</Text>
